...
class EpisodeFileRepr(object):
    ...
    def move(self, tvdb_instance, dest, cleanup=None):
        ...
        dest_dir = os.path.join(dest, self._file_info[
            "title"], "Season %d" % self._file_info["season"])
    ...
    def _fetch_episode_name(self, tvdb_instance):
        ...
        if not search_results:
            raise tvdb_exceptions.tvdb_shownotfound, "Show not found! Giving up. (%s)" % self._file_info[
                "name"]
            ...
            if transform:
                for transformation in transform:
                    try:
                        ...
                        self._log.warn(
                            "Bad transformation string! Needs to be in A=B format.")
                        continue

                    if this in title:
                        self._log.info(
                            "Transforming %s to %s..." % (this, that))

                        new_title = title.replace(this, that)
                        self._log.info("%s became %s" % (title, new_title))
                        title = new_title

            self._file_info = {"name": self._name, "title": title.replace(
                ".", " ").strip(), "season": None, "episode": None}
            if s1 and e1:
