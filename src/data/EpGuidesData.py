...
class EpGuidesData(Data.Data):
    ...
    def get_episode_name(self, show, season, episode):
        ...
        """
        self.logger.info(
            u"Retrieving show {0:s} from season {1:d} episode {2:d}"
            .format(show, season, episode))
        try:
            ep = self.data[show][season][episode]
            self.logger.info(
                u"Returning episodes for {0:s} from data".format(show))
            return ep
        except KeyError:
            self.logger.info(
                u"{0:s} is not cached, fetching from the internet".format(show))

        query = {"q": u"allintitle: site:epguides.com {0:s}".format(show),
                 "userip": socket.gethostbyname(socket.gethostname())}
        search_url = u"http://ajax.googleapis.com/ajax/services/" \
                     u"search/web?v=1.0&{0:s}".format(urllib.urlencode(query))

        self.logger.info(u"Searching at {0:s}".format(search_url))
        search_results = json.loads(get_url_contents(search_url))

        if not (200 == search_results["responseStatus"] and not (
                "estimatedResultCount" not in
                search_results["responseData"]["cursor"])):
            self.logger.error(u"Show not found! Response details: {0:s}".format(
                search_results["responseDetails"]))
            return None

        epguides_url = search_results["responseData"]["results"][0]["url"]
        self.logger.info(u"Looking for CSV listing at {0:s}".format(
            epguides_url))
        ...
        try:
            ...
            self.logger.error("Couldn't find CSV listing for {0:s} at {1:s}. "
                              "Aborting.".format(show, epguides_url))
            return None
        ...
        for i in xrange(0, len(csv)):
            ...
            try:
                ...
                self.logger.warning(u"Season or episode number are missing. "
                                    u"Skipping this episode: {0:s}".format(
                    rowdict["title"]))
                continue
